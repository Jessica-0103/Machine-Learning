...
# Jessica Chouhan 0827CI201087   
# Initialize variables learning_rate = 0.1 iterations = 5000  
N = y_train.size  
...
= 3    
 results = pd.DataFrame(columns=["mse", "accuracy"])   
  ...
  Backpropagation  
# Jessica Chouhan 0827CI201087   for itr in range(iterations):      
...
Testing using CIFAR 100 Multiclass Classification Dataset: 
...
import cifar100 from tensorflow.keras.models 
import Sequential from tensorflow.keras.layers 
import Dense, Dropout, BatchNormalization from tensorflow.keras.callbacks 
import EarlyStopping  

(x_train, y_train), (x_test, y_test) = cifar100.load_data()   
# Normalize the input images to have pixel values between 0 and 
1 x_train = x_train.astype('float32') / 255.0 x_test = x_test.astype('float32') / 255.0    
...
# Input layer     tf.keras.layers.Flatten(input_shape=(32, 32, 3)),  
...
# Train the model  history = model.fit(x_train, y_train, epochs=50, 
batch_size=64, validation_split=0.2, callbacks=[early_stop])   
# Evaluate the model on test set  test_loss, test_acc = model.evaluate(x_test, y_test)   
# Print the test accuracy  print("Test accuracy:", test_acc)   
